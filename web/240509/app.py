...
#   html 문서를 한줄한줄 읽어서 str형태로 parsing 후 유저에게 되돌려주는 형태
#   html 문서에서 {{변수명}}, {% python code %}를 사용 가능
#   f-string과 비슷한 기능 f"{x}"
...
## api 생성
# router: 네이게이터 함수
# route()함수 안에 인자값과 루트주소와 연결된 주소 값으로 요청 시 아래의 함수를 호출
# ex) @app.route('/view_info') --> localhost:8080/view_info 주소로 요청 시  
@app.route('/')
def index():
...
@app.route('/first')
def first():
    # tamplates 폴더 안에 있는 page1.html 파일을 로드하여 parsing
...
@app.route('/chart')
def chart():
    # select키로 들어오는 데이터에 따라서 로드하는 csv파일을 변경 
    # type키는 라인, 바를 선택
    ...
    return render_template('chart.html', _x = input_x, _y = input_y, _type = input_type)
...
# 127.0.0.1:8080 대신 localhost:8080이라 써도 된다